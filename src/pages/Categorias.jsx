  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold text-slate-900 dark:text-white">Categorias</h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-0.5">Organize seus lançamentos por categoria</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <input type="color" value={form.cor} onChange={set('cor')}
                className="w-12 h-10 rounded-lg border border-slate-200 dark:border-slate-600 cursor-pointer p-1 dark:bg-slate-800" />
              <span className="text-sm text-slate-500 dark:text-slate-400">{form.cor}</span>
            </div>
          </div>
          ...
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={() => setModalOpen(false)}
              className="flex-1 border border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 py-2.5 rounded-xl text-sm font-medium">Cancelar</button>
            <button type="submit" disabled={formLoading}
              className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white py-2.5 rounded-xl text-sm font-medium disabled:opacity-60 flex items-center justify-center gap-2">
...
function CategoryCard({ cat, deletandoId, setDeletandoId, handleDelete, isRoot }) {
  return (
    <div className={`bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-4 flex items-center justify-between group ${isRoot ? '' : 'py-3'}`}>
      <div className="flex items-center gap-3">
        <div className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: cat.cor || '#6366f1' }} />
        <div>
          <p className="text-sm font-medium text-slate-800 dark:text-slate-200">{cat.nome}</p>
