...
function CustomTooltipPie({ active, payload }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-100 dark:border-slate-700 p-3 text-sm">
      ...
      <p className="text-slate-600 dark:text-slate-300">{fmt(payload[0].value)}</p>
    </div>
  )
}
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-2 py-1.5 shadow-sm">
          <button onClick={prevMes} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors">
            <ChevronLeft size={16} className="text-slate-500" />
          </button>
          <span className="text-sm font-medium text-slate-700 dark:text-slate-200 min-w-[120px] text-center">{mesLabel}</span>
          <button onClick={nextMes} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors">
      ...
      {loading ? (
        ...
        <>
            ...
            {patrimonio && (
              <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-5">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">Patrimônio Líquido</span>
                </div>
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-5">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">Projeção do Mês</span>
                  </div>
                ...
                <div className="space-y-1.5 text-xs">
                  <div className="flex justify-between text-slate-500 dark:text-slate-400">
                    ...
                    <span className={previsao.saldoAtual >= 0 ? 'text-slate-700 dark:text-slate-300 font-medium' : 'text-red-500 font-medium'}>
                  ...
                  {previsao.recorrenciasPendentes > 0 && (
                    <>
                      ...
                      <div className="pt-1 border-t border-slate-100 dark:border-slate-800 text-slate-400">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-5 mb-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">Reserva de Emergência</span>
              </div>
              {!editandoReserva && (
                <button
            ...
            {editandoReserva ? (
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  className="flex-1 px-3 py-1.5 border border-slate-200 dark:border-slate-600 rounded-xl text-sm dark:bg-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-amber-400"
                />
                <button onClick={salvarReserva} className="p-1.5 bg-amber-500 hover:bg-amber-600 text-white rounded-lg">
                  <Check size={13} />
                </button>
                <button onClick={() => setEditandoReserva(false)} className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
            <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-6">
              <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-4">Evolução Anual {ano}</h2>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-6">
              <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-4">Top Despesas do Mês</h2>
          ...
          {orcamentos.length > 0 && (
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-6 mb-4">
              <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-4">Orçamentos do Mês</h2>
              <div className="space-y-3">
                {orcamentos.map(o => (
                  <div key={o.id}>
                    <div className="flex justify-between text-sm mb-1">
                      <span className="font-medium text-slate-700 dark:text-slate-300">{o.categoriaNome}</span>
                      <span className="text-slate-500 dark:text-slate-400">{fmt(o.valorGasto)} <span className="text-slate-300 dark:text-slate-600">/</span> {fmt(o.valorLimite)}</span>
                    </div>
          ...
          {/* Últimos Lançamentos */}
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-6">
            <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-4">Últimos Lançamentos</h2>
                ...
                {recentesLancamentos.map((l) => (
                  <div key={l.id} className="flex items-center justify-between py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-slate-800 dark:text-slate-200">{l.descricao}</p>
