function SkeletonLine({ w = 'w-full', h = 'h-4' }) {
  return <div className={`${w} ${h} bg-slate-200 dark:bg-slate-700 rounded animate-pulse`} />
}
    ...
    <div className="space-y-3">
      {[...Array(rows)].map((_, i) => (
        <div key={i} className="flex items-center gap-3 p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800">
          <div className="w-9 h-9 rounded-xl bg-slate-200 dark:bg-slate-700 animate-pulse shrink-0" />
